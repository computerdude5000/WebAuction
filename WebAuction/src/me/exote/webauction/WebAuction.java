...
public class WebAuction extends JavaPlugin{
    ...
    public static MySQL mysql; // MySQL handler
	...
	public void loadConfiguration(){
		...
		plugin.getConfig().addDefault("MySQL.Passord", "password123");
		plugin.getConfig().addDefault("MySQL.Port", "3306");
		plugin.getConfig().addDefault("MySQL.Database", "minecraft");
		plugin.getConfig().addDefault("Misc.ReportSales", false);
		plugin.getConfig().addDefault("Misc.SignDelay", 1000);
	...
	public void onEnable() {
		...
		dbUser = plugin.getConfig().getString("MySQL.Username");
		dbPass = plugin.getConfig().getString("MySQL.Passord");
		dbPort = plugin.getConfig().getString("MySQL.Port");
		dbDatabase = plugin.getConfig().getString("MySQL.Database");
		getMessages = plugin.getConfig().getBoolean("Misc.ReportSales");
		signDelay = plugin.getConfig().getInt("Misc.SignDelay");
		...
		if (this.dbPort.equals(null)) {this.log.severe(this.logPrefix + "MySQL port is not defined"); }
		...
		try {
			if (this.mysql.checkConnection()) { // Check if the Connection was successful
				...
				if (!this.mysql.checkTable("WA_Players")) {
				...
				if (!this.mysql.checkTable("WA_StorageCheck")) {
				...
				if (!this.mysql.checkTable("WA_WebAdmins")) {
				...
				if (!this.mysql.checkTable("WA_Items")) {
				...
				if (!this.mysql.checkTable("WA_Enchantments")) {
				...
				if (!this.mysql.checkTable("WA_EnchantLinks")) {
				...
				if (!this.mysql.checkTable("WA_Auctions")) {
				...
				if (!this.mysql.checkTable("WA_SellPrice")) {
				...
				if (!this.mysql.checkTable("WA_MarketPrices")) {
				...
				if (!this.mysql.checkTable("WA_Mail")) {
				...
				if (!this.mysql.checkTable("WA_RecentSigns")) {
				...
				if (!this.mysql.checkTable("WA_ShoutSigns")) {
				...
				if (!this.mysql.checkTable("WA_SaleAlerts")) {
		...
		try {
			result1 = WebAuction.mysql.query(query1);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			//e.printStackTrace();
		}
		...
		try{
			this.getServer().getScheduler().scheduleAsyncRepeatingTask(this, new Runnable() {
				...
				public void run() {
					...
					try {
						result2 = WebAuction.mysql.query(queryLatest);
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
						...
						if (result2 != null){
							...
							if (lastAuction < result2.getInt("id"))
							{
			    				...
			    				while (it.hasNext()){
									...
									try {
										WebAuction.mysql.query(queryRemove);
									} catch (Exception e) {
										e.printStackTrace();
									} 
			    				}
		...
		try{
			this.getServer().getScheduler().scheduleAsyncRepeatingTask(this, new Runnable() {
				...
				public void run() {
					...
					int resultLength = 0;
					try {
						result2 = WebAuction.mysql.query(query2);
						resultTest = result2;
